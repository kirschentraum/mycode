...
TCHAR szAppName[] = L"PopPad2";

inline int AskConfirmation(HWND hwnd)
{
    return MessageBoxW(hwnd, L"Really want to clos PopPad2?", szAppName, MB_YESNO|MB_ICONQUESTION);
}
...
LRESULT WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITMENUPOPUP:
        if (lParam == 1)
        {
            EnableMenuItem((HMENU)wParam, IDM_EDIT_UNDO, SendMessageW(hwndEdit, EM_CANUNDO, 0, 0) ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, IDM_EDIT_PASTE, IsClipboardFormatAvailable(CF_TEXT) ? MF_ENABLED : MF_GRAYED);
            iSelect = SendMessageW(hwndEdit, EM_GETSEL, 0, 0);
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR szCmdLine, _In_ int iCmdShow)
{
    ...
    wndclass.hIcon = LoadIcon(hInstance, MAKEINTRESOURCEW(IDI_ICON2));
    wndclass.hCursor = LoadCursorW(NULL, IDC_ARROW);
    wndclass.hbrBackground = CreateSolidBrush(0);
    wndclass.lpszMenuName = MAKEINTRESOURCEW(POPPAD2);
    ...
    hwnd = CreateWindowW(szAppName, szAppName, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);
    ShowWindow(hwnd, iCmdShow);
    UpdateWindow(hwnd);
    hAccel = LoadAcceleratorsW(hInstance, MAKEINTRESOURCEW(POPPAD2_ACC));
