...
void AddHelpToSys(HINSTANCE hInstance, HWND hwnd)
{
    ...
    hBitmap = StretchBitmap(LoadBitmap(hInstance, MAKEINTRESOURCE(BITMAPHELP)));
...
HMENU CreateMyMenu(HINSTANCE hInstance)
{
    ...
    hMenuPopup = LoadMenu(hInstance, MAKEINTRESOURCE(MENUFILE));
    hBitmap = StretchBitmap(LoadBitmap(hInstance, MAKEINTRESOURCE(BITMAPFILE)));
    AppendMenuW(hMenu, MF_BITMAP | MF_POPUP, (int)hMenuPopup,
        (LPCWSTR)hBitmap);

    hMenuPopup = LoadMenu(hInstance, MAKEINTRESOURCE(MENUEDIT));
    hBitmap = StretchBitmap(LoadBitmap(hInstance, MAKEINTRESOURCE(BITMAPEDIT)));
    AppendMenuW(hMenu, MF_BITMAP | MF_POPUP, (int)hMenuPopup,
        (LPCWSTR)hBitmap);
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        AppendMenuW(hMenuPopup, MF_BITMAP, IDM_FONT_COUR + i,
            (LPCWSTR)hBitmap);
    }

    hBitmap = StretchBitmap(LoadBitmap(hInstance, MAKEINTRESOURCE(BITMAPFONT)));
...
{
    const static TCHAR* szFaceName[3] = { TEXT("Courier New"), TEXT("Arial"),
