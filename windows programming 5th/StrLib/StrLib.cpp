...
EXPORT BOOL CALLBACK AddStringW (PCWSTR pStringIn)
{
     ...
     wcscpy_s (pString, wcslen(pStringIn)+1, pStringIn) ;
     _wcsupr_s (pString, wcslen(pString) + 1) ;
     ...
     wcscpy_s (szStrings[i], wcslen(pString)+ 1, pString) ;
     iTotal++ ;
...
EXPORT int CALLBACK GetStringsA (GETSTRCB pfnGetStrCallBack, PVOID pParam)
{
     ...
     for (i = 0 ; i < iTotal ; i++)
     {
          ...
          bReturn = pfnGetStrCallBack ((PCTSTR)pAnsiStr, (PVOID)pParam) ;
