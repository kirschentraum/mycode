...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        hwndText = CreateWindow(TEXT("static"), szBuffer,
            ...
            hwnd, (HMENU)(long long)ID_TEXT,
            (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE),
            NULL);

        OldList = (WNDPROC)SetWindowLongPtr(hwndList, GWLP_WNDPROC,
            (LPARAM)ListProc);
    ...
    case WM_COMMAND:
        ...
        {
            if (LB_ERR == (i = (DWORD)SendMessageW(hwndList, LB_GETCURSEL, 0, 0)))
                break;

            SendMessageW(hwndList, LB_GETTEXT, i, (LPARAM)szBuffer);
    ...
    case WM_PAINT:
        ...
        readResult=ReadFile(hFile, buffer, MAXREAD, &i, NULL);
        if (!readResult)
            MessageBoxW(hwnd, L"File Open Error.", L"Error", MB_OK);
        CloseHandle(hFile);
