...
#define BUFFER(x,y) *(pBuffer + (long long)y * (long long)cxBuffer + x)
...
LRESULT WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
        ...
        pBuffer = (TCHAR*)malloc((long long)cxBuffer * (long long)cyBuffer * sizeof(TCHAR));
        if (pBuffer == NULL)
        {
            MessageBoxW(hwnd, L"No enough memory", L"Error", MB_OK);
            return 0;
        }
